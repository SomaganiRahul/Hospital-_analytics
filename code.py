...
# Load data
df = pd.read_csv('C:\\Users\\nanis\\OneDrive\\Desktop\\hda\\hda\\finaldata.csv')
...
if not st.session_state['authenticated']:
    st.title('Hospital Data Analysis Login')
        ...
        def generate_report(start_date, end_date):
            try:
                ...
                if most_common_region_count >5:  # Example threshold
                    doc.add_paragraph(f"Consider increase the number of doctors available for patients from {most_common_region} due to high patient inflow.")
